...
-(void) saveSignal:(CXSignal*)oneRootSignal
{
    ...
    else
    {
        ...
        {
            if( [rootSignal hasSameDNA:oneRootSignal] )
            {
                [oneRootSignal removeSignal];
                return;
            }
        }
         ...
         transformer:(id(^)(id value))transformer
{
    ...
    for( NSInteger i = _KVOSignals.count-1; i>=0; i-- )
    {
        CXSignal* eachRootSignal = _KVOSignals[i];
        CXSignal* similerSignal = [eachRootSignal searchSignal:signal];
        if( similerSignal )
        {
            [_KVOSignals removeObject:eachRootSignal];
            [eachRootSignal removeSignal];
        }
    }
